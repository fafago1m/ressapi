...
function generateTransactionId() {
  return `ACTSTOREID-${crypto.randomBytes(2).toString('hex').toUpperCase()}`;
}
...
async function elxyzFile(buffer) {
  ...
  const { directLink } = await service.uploadFromBinary(buffer, 'actstoreid.png');
  return directLink;
}
...
module.exports = [
  {
    ...
    async run(req, res) {
      ...
      try {
        ...
        if (!login.results) {
          console.error('OrderKuota Login Error:', login);
          return res.json({ status: false, error: 'Upstream error', details: login });
        }
  ...
  },
  {
    ...
    async run(req, res) {
      ...
      try {
        ...
        if (!login.results) {
          console.error('OrderKuota GetToken Error:', login);
          return res.json({ status: false, error: 'Upstream error', details: login });
        }
        res.json({ status: true, result: login.results });
      } catch (err) {
  ...
  },
  {
    ...
    async run(req, res) {
      ...
      try {
        ...
        if (!login.qris_history) {
          console.error('OrderKuota MutasiQR Error:', login);
          return res.json({ status: false, error: 'Upstream error', details: login });
        }
  ...
  },
  {
    ...
    async run(req, res) {
      ...
      try {
        ...
        let login = await ok.getTransactionQris();
        if (!login.qris_history) {
          console.error('OrderKuota Profile Error:', login);
          return res.json({ status: false, error: 'Upstream error', details: login });
        }
        // Assuming profile might want something else, but original code just returned login (which was modified in mutasi but here is raw?)
        // actually original code: login = login; res.json({status: true, result: login});
        // So we just return the full response or qris_history? 
        // Original code was just `login = login` which does nothing.
        // Let's assume we return the whole thing.
        res.json({ status: true, result: login });
      } catch (err) {
        res.status(500).json({ status: false, error: err.message });
      }
